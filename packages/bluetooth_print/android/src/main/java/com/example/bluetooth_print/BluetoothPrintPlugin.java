...
public class BluetoothPrintPlugin implements FlutterPlugin, ActivityAware, MethodCallHandler, RequestPermissionsResultListener {
  ...
  private void tearDown() {
    Log.i(TAG, "teardown");
    context = null;
    ActivityPluginBinding localBinding = activityBinding;
    activityBinding = null;
    if (localBinding != null) {
      localBinding.removeRequestPermissionsResultListener(this);
    }
    stopScan();
  ...
  private void getDevices(Result result){
    ...
    try {
      ...
      Log.w(TAG, "Unable to access bonded devices", exception);
    }
  ...
  private void invokeMethodUIThread(final String name, final BluetoothDevice device)
  {
    ...
    if (address == null || !discoveredDevices.add(address)) {
      return;
    }
  ...
  private void startScan() throws IllegalStateException {
    ...
    // Start classic discovery for legacy thermal printers that do not support BLE advertising.
    try {
    ...
    if (scanner != null) {
      ...
      Log.w(TAG, "Bluetooth LE scanner unavailable; relying on classic discovery");
    }
  }
  ...
  private void stopScan() {
    ...
    if (mBluetoothAdapter != null && mBluetoothAdapter.isDiscovering()) {
      try {
        ...
        Log.w(TAG, "Unable to cancel discovery", e);
      }
    }
    discoveredDevices.clear();
  }
  ...
  private void registerDiscoveryReceiver() {
    if (application == null || isReceiverRegistered) {
      return;
    }
